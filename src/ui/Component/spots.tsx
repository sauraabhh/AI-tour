...
function spots() {
  ...
  const handleBook = (attraction) => {
    ...
    const price = attraction['Entrance Fee in INR'] || 'N/A';
    alert(`Booking Attraction: ${name} at ₹${price}`);
  };
          ...
          attractions.map((attraction, index) => (
            ...
            >
              ...
              <p>
                ...
                {attraction['Best Time to visit'] || 'Time not available'}
              </p>
              <p><strong>Weekly Off:</strong> {attraction['Weekly Off'] || 'Not available'}</p>
              <p>
                <strong>Time Needed:</strong> {attraction['time needed to visit in hrs']} hrs
              </p>
              {attraction['Entrance Fee in INR'] && (
                <button onClick={() => handleBook(attraction)}>
                  Book for ₹{attraction['Entrance Fee in INR']}
                </button>
              )}
            </div>
          ))
